...
void ImageConverter::depthCb(const std::shared_ptr<sensor_msgs::msg::Image> msg)
{
    ...
    if (enable_get_depth)
    {
        ...
        if (num_depth_points != 0)
        {
            ...
            if (abs(distance - this->minDist) < 0.1)
                linear_speed = 0;
            else
                linear_speed = -linear_PID->compute(this->minDist, distance); //-linear_PID->compute(minDist, distance)
        }
