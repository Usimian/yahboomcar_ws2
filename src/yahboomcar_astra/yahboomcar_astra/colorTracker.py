...
# import math
import time
from yahboomcar_astra.astra_common import simplePID
...
class color_Tracker(Node):
    ...
    def PID_init(self):
        self.linear_pid = simplePID(
            self.linear_PID[0] / 1000.0, self.linear_PID[1] / 1000.0, self.linear_PID[2] / 1000.0
        )
        self.angular_pid = simplePID(
            self.angular_PID[0] / 100.0, self.angular_PID[1] / 100.0, self.angular_PID[2] / 100.0
        )
    ...
    def depth_img_Callback(self, msg):
        ...
        if self.Center_r != 0:
            ...
            distance = [0, 0, 0, 0, 0]
            if (
                0 < int(self.Center_y - 3)
                and int(self.Center_y + 3) < 480
                and 0 < int(self.Center_x - 3)
                and int(self.Center_x + 3) < 640
            ):
