...
def read_HSV(rf_path):
    ...
    if len(list) != 6:
        return ()
    hsv = ((int(list[0]), int(list[1]), int(list[2])), (int(list[3]), int(list[4]), int(list[5])))
    rf.flush()
    return hsv


# Define the function, the first parameter is the zoom ratio,
#  and the second parameter is a tuple or list of pictures to be displayed
def ManyImgs(scale, imgarray):
    rows = len(imgarray)  # Length of tuple or list
    cols = len(imgarray[0])
    # If imgarray is a list, return the number of channels of the first image in the list,
    #  if it is a tuple, return the length of the first list contained in the tuple
    ...
    if rowsAvailable:
        ...
        hor = [imgBlank] * rows
        # The same size as the first picture,
        #  and the same number of horizontal blank images as the tuple contains the list
        for x in range(0, rows):
