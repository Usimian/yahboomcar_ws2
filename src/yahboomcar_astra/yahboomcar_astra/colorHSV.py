...
# import math
import time
from yahboomcar_astra.astra_common import color_follow, ManyImgs, read_HSV, write_HSV
import cv2 as cv
    ...
    def process(self, rgb_img, action):
        ...
        if self.Track_state == "tracking":
            ...
            if self.point_pose[0] != 0 and self.point_pose[1] != 0:
                threading.Thread(
                    target=self.execute, args=(self.point_pose[0], self.point_pose[1], self.point_pose[2])
                ).start()
        else:
