...
class Rosmaster(object):
    ...
    def __parse_data(self, ext_type, ext_data):
        ...
        # Encoder data on all four wheels
            ...
            if ext_type == self.FUNC_UART_SERVO:
                ...
                idx = struct.unpack("B", bytearray(ext_data[0:1]))[0]
                self.__akm_def_angle = struct.unpack("B", bytearray(ext_data[1:2]))[0]
                self.__akm_readed_angle = True
                if self.__debug:
                    print("FUNC_AKM_DEF_ANGLE:", idx, self.__akm_def_angle)
    ...
    # Limit the PWM duty ratio value of motor input, value=127,
    #   keep the original data, do not modify the current motor speed
    ...
    # enable=True, The underlying expansion board sends four different packets of data every 10 milliseconds,
    #   so each packet is refreshed every 40 milliseconds.
    ...
    # RGB programmable light belt control, can be controlled individually or collectively,
    #   before control need to stop THE RGB light effect.
    ...
    def set_colorful_lamps(self, led_id, red, green, blue):
        ...
        try:
            ...
            g = int(green) & 0xFF
            b = int(blue) & 0xFF
            cmd = [self.__HEAD, self.__DEVICE_ID, 0x00, self.FUNC_RGB, idx, r, g, b]
            cmd[2] = len(cmd) - 1
            checksum = sum(cmd, self.__COMPLEMENT) & 0xFF
            cmd.append(checksum)
            self.ser.write(cmd)
    ...
    # Effect =[0, 6], 0: stop light effect, 1: running light, 2: running horse light, 3:
    #   breathing light, 4: gradient light, 5: starlight, 6: power display
    ...
    # Adjust =True Activate the gyroscope auxiliary motion direction.
    #   If =False, the function is disabled.(This function is not enabled)
    ...
    # KP ki kd = [0, 10.00]
    # forever=True for permanent, =False for temporary.
    # Since permanent storage needs to be written into the chip flash, which takes a long time to operate,
    #   delay is added to avoid packet loss caused by MCU.
    ...
    # Otherwise, all connected bus actuators will be set to the same ID, resulting in confusion of control
    ...
    # Run the following command to set the mid-bit deviation of the manipulator:
    #   servo_id=0 to 6, =0 Restore the factory default values
    def set_uart_servo_offset(self, servo_id):
        """set_uart_servo_offset."""
        try:
    ...
    def set_akm_default_angle(self, angle, forever=False):
        """set_akm_default_angle."""
        try:
            if int(angle) > 120 or int(angle) < 60:
                return
            idx = self.__AKM_SERVO_ID
            state = 0
            if forever:
                state = 0x5F
                self.__akm_def_angle = angle
            cmd = [self.__HEAD, self.__DEVICE_ID, 0x00, self.FUNC_AKM_DEF_ANGLE, idx, int(angle), state]
            cmd[2] = len(cmd) - 1
            checksum = sum(cmd, self.__COMPLEMENT) & 0xFF
            cmd.append(checksum)
            self.ser.write(cmd)
    ...
    # ctrl_car=False, =True
    # Control the steering Angle of ackman type (R2) car relative to the default Angle,
    # negative for left and positive for right, Angle =[-45, 45]
    # ctrl_car=False, only control the steering gear Angle, =True,
    # control the steering gear Angle and modify the speed of the left and right motors.
    def set_akm_steering_angle(self, angle, ctrl_car=False):
        """set_akm_steering_angle."""
        try:
            if int(angle) > 45 or int(angle) < -45:
                return
            idx = self.__AKM_SERVO_ID
            if ctrl_car:
                idx = self.__AKM_SERVO_ID + 0x80
            cmd = [self.__HEAD, self.__DEVICE_ID, 0x00, self.FUNC_AKM_STEER_ANGLE, idx, int(angle) & 0xFF]
            cmd[2] = len(cmd) - 1
            checksum = sum(cmd, self.__COMPLEMENT) & 0xFF
            cmd.append(checksum)
            self.ser.write(cmd)
