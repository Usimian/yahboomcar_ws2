...
class YahboomCarPatrol(Node):
    ...
    def advancing(self, target_distance):
        ...
        move_cmd = Twist()
        self.distance = sqrt(pow((self.position.x - self.x_start), 2) + pow((self.position.y - self.y_start), 2))
    ...
    def Spin(self, angle):
        ...
        move_cmd = Twist()
        if abs(self.error) < self.RotationTolerance or self.Switch is False:
            self.pub_cmdVel.publish(Twist())
            self.turn_angle = 0.0
            """self.Switch  = rclpy.parameter.Parameter('Switch',rclpy.Parameter.Type.BOOL,False)
            all_new_parameters = [self.Switch]
    ...
    def get_odom_angle(self):
        ...
        try:
            ...
            cacl_rot = PyKDL.Rotation.Quaternion(
                rot.transform.rotation.x, rot.transform.rotation.y, rot.transform.rotation.z, rot.transform.rotation.w
            )
