...
class CalibrateAngular(Node):
    ...
    def __init__(self, name):
        ...
        # self.tf_listener.waitForTransform(self.odom_frame, self.base_frame, rclpy.time.Time(),
    ...
    def on_timer(self):
        ...
        if self.start_test:
            ...
            if self.start_test and (abs(self.error) > self.tolerance or self.error == 0):
                ...
                self.odom_angle = self.get_odom_angle()
                self.delta_angle = self.odom_angular_scale_correction * self.normalize_angle(
                    self.odom_angle - self.first_angle
                )
